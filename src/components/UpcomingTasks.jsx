...
const UpcomingTasks = ({ handleOnEdit }) => {
  const { loader, setloader, task, handleOnDelete, handleOnComplete } = useContext(TaskContext)


  return (
    <div>
      <p className='capitalize justify-center flex my-8 text-2xl font-semibold'>
        Upcoming Tasks
      </p>
                  ...
                  <span className='flex items-center ml-auto text-[22px]'>
                    <span className='flex shadow-none px-2 bg-slate-200 rounded-lg mr-1'>
                      ...
                      <p className='text-sm mr-1'>
                        {item.time} | {item.date && item.date.slice(0, 10)}
                      </p>
                    </span>
                    <MdDeleteForever
